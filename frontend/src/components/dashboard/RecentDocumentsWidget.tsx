...
export function RecentDocumentsWidget({ documents, onSelectDocument }: RecentDocumentsWidgetProps) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <h3 className="text-sm font-semibold text-slate-800">Documenti recenti</h3>
      <div className="mt-3 overflow-x-auto">
        {documents.length === 0 ? (
          <p className="text-sm text-slate-500">Nessun documento</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {documents.map((d) => (
                ...
                >
                  <td className="py-2 pr-2 font-medium text-slate-800">{d.title}</td>
                  <td className="py-2 pr-2 text-slate-600">{STATUS_LABELS[d.status] || d.status}</td>
                  <td className="py-2 pr-2 text-slate-500">
                    {new Date(d.updated_at).toLocaleDateString('it-IT')}
                  </td>
                  <td className="py-2 text-slate-500">{d.created_by_email || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
